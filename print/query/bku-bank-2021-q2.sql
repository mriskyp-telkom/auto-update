...
from 
	(
		select 
			...
			,uraian       
			,no_bukti
			,case when id_ref_bku in (2,5,6,8,22,25,26,28) then saldo else 0 end as debit
			,case when id_ref_bku in (3,7,14,15,23,27,34,35) then saldo else 0 end as credit
			,(
				select 
					SUM(case when id_ref_bku in (2,5,6,8,22,25,26,28) then saldo else 0 end)
					-SUM(case when id_ref_bku in (3,7,14,15,23,27,34,35) then saldo else 0 end) 
				from 
					rpt_bku as a
				where 
					a.rowid<=b.rowid 
		...
		from 
			rpt_bku as b 
		where 
			id_ref_bku in (2,5,6,8,22,25,26,28,3,7,14,15,23,27,34,35) 
