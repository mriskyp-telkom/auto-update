select 
       ...
       ,case when ri.id_ref_indikator = 2 then 
             (select ifnull(sum(r.jumlah),0)  
               ...
               where r.id_anggaran=:id_anggaran and r.soft_delete=0 
             )
...
from 
     ...
     left join (
          select 
                 i2.id_ref_indikator
                 ,i2.tolak_ukur
                 ,i2.capaian as target_kinerja 
          from 
                 anggaran a 
                 join rencana_kerja_anggaran rka on a.id_anggaran = rka.id_anggaran 
                 join indikator i2 on rka.id_rka = i2.id_rka
          where 
                a.id_anggaran=:id_anggaran 
                and a.soft_delete=0 
                and rka.soft_delete=0 
                and i2.soft_delete=0) i          
