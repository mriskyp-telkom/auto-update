select 
       ...
       ,sum(case when dr.id_periode = 1 then dr.jumlah else 0 end) as tahap1
       ,(sum(case when dr.id_periode = 2 then dr.jumlah else 0 end) + ((sum(case when dr.id_periode = 3 then dr.jumlah else 0 end)/3)*2) )as tahap2   
       ,(sum(case when dr.id_periode = 4 then dr.jumlah else 0 end) + ((sum(case when dr.id_periode = 3 then dr.jumlah else 0 end)/3)*1) )as tahap3   
       ,1 as color_id
	   ,null as volume
	   ,null as satuan
	   ,null as tariff
