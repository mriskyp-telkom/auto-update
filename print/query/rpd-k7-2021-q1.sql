    ...
    where 
      bx.id_penjab in (
        ...
        where 
          ...
          and (
			case 
				when :id_periode = 1 then 6 
				when :id_periode = 2 then 12 
				when :id_periode = 11 then 3 
				when :id_periode = 12 then 8 
				when :id_periode = 13 then 12
        when :id_periode > 80 then :id_periode-80 
			end
			) >= cast(strftime('%m', tanggal_mulai) as int) 
          and (
			case 
				when :id_periode = 1 then 6 
				when :id_periode = 2 then 12 
        ...
        when :id_periode > 80 then :id_periode-80 
			...
			) <=  cast(strftime('%m', tanggal_selesai) as int)
      )
    ) p  
