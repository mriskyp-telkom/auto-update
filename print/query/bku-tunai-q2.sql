...
from 
     ...
     select 
       ...
       ,uraian       
       ,no_bukti
       ,case when id_ref_bku in (3,9,10) then saldo else 0 end as debit
       ,case when id_ref_bku in (4,5,11) then saldo else 0 end as credit
       ,(select SUM(case when id_ref_bku in (3,9,10) then saldo else 0 end)-SUM(case when id_ref_bku in (4,5,11) then saldo else 0 end) from (select * from kas_umum order by tanggal_transaksi,create_date) as a where id_anggaran=:id_anggaran and tanggal_transaksi >= (select tanggal_aktivasi from aktivasi_bku where soft_delete=0 and id_anggaran=:id_anggaran and id_periode=:id_periode) and tanggal_transaksi <=(select tanggal_finish from aktivasi_bku where soft_delete=0 and id_anggaran=:id_anggaran and id_periode=:id_periode) and soft_delete=0 and a.tanggal_transaksi<=b.tanggal_transaksi and a.create_date<=b.create_date and a.rowid<=b.rowid )+0.0 as balance
     from kas_umum as b 
     ...
     and id_ref_bku in (3,4,5,9,10,11) 
